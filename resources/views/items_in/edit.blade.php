...
<main>
    <section class="flex flex-col items-start justify-start p-4 rounded-lg shadow-md bg-white border border-pink-200">
        ...
        <form action="{{ route('items-in.edit', ['id'=>$id]) }}" method="POST" class="w-full" x-data="{
        products: {{ Illuminate\Support\Js::from($products) }},
        items: {{ Illuminate\Support\Js::from($itemIn->details) }},
        submit() {
            ...
            <div class="inline-flex justify-between items-center pb-4 mb-4 w-full space-x-4 border-b border-pink-300">
                <div class="flex flex-col w-full max-w-lg">
                    <select name="supplier_id" class="p-2 border border-pink-300 rounded">
                        ...
                        <option value="{{ $supplier->id }}" {{ old('supplier_id') == $supplier->id || $itemIn->supplier->id == $supplier->id ? 'selected' : '' }}>
                            {{ $supplier->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="flex flex-col w-fit">
                    ...
                    <div class="flex items-center space-x-4">
                        <label class="flex items-center space-x-2">
                            <input type="radio" name="is_paid" value="1" {{ old('is_paid') == '1' || $itemIn->is_paid == '1' ? 'checked' : '' }} class="form-radio text-pink-600">
                            <span>Dibayar</span>
                        </label>
                        <label class="flex items-center space-x-2">
                            <input type="radio" name="is_paid" value="0" {{ old('is_paid') == '0' || $itemIn->is_paid == '0' ? 'checked' : '' }} class="form-radio text-pink-600">
