...
<main x-data="{
    ...
    sortBy: 'id',
    sortAsc: true,
    items_in: {{ Illuminate\Support\Js::from($itemsIn->map(function($itemIn) {
            return [
                'id' => $itemIn->id,
                'created_at' => $itemIn->created_at->format('d/m/Y H:i'),
                'supplier' => $itemIn->supplier->name,
                'total_item' => $itemIn->total_item,
                'total_price' => number_format($itemIn->total_price, 0, ',', '.'),
                'is_paid' => $itemIn->is_paid,
                'details' => [
                    'product' => $itemIn->details->map(function($detail) {
                        return [
                    ...
                    'total_price' => number_format($itemIn->details->sum(fn($d) => $d->quantity * $d->price), 0, ',', '.'),
                    ]
            ];
        })) }},
    page: 1,
    ...
    <section class="flex flex-col gap-6 items-start justify-start p-6 rounded-xl shadow-lg bg-white border border-pink-200 w-full max-w-6xl mx-auto mt-6">
        <div class="flex flex-col md:flex-row w-full justify-between items-center gap-4">
            <a href="{{ route('items-in.create') }}" class="bg-pink-500 text-white px-6 py-3 rounded-lg shadow hover:bg-pink-600 transition-colors font-semibold">
        ...
        <div class="overflow-x-auto w-full">
            <table class="w-full table-auto border-collapse rounded-lg overflow-hidden shadow border border-pink-300">
                ...
                <tbody>
                    <template x-if="paged.length">
                        <template x-for="row in paged" :key="row.id">
                            <tr class="hover:bg-pink-50 transition-colors">
                                <td class="border border-pink-300 px-4 py-2 text-center font-mono" x-text="row.id"></td>
                                <td class="border border-pink-300 px-4 py-2 text-center" x-text="row.created_at"></td>
                                <td class="border border-pink-300 px-4 py-2" x-text="row.supplier"></td>
                                <td class="border border-pink-300 px-4 py-2 text-center" x-text="row.total_item"></td>
                                <td class="border border-pink-300 px-4 py-2 text-right">Rp <span x-text="row.total_price"></span></td>
                                ...
                                <td class="border border-pink-300 px-4 py-2 text-center">
                                    <div class="inline-flex">
                                        <button
                                            @click="$store.modal.open(row)"
                                            class="bg-pink-100 text-pink-700 px-3 py-1 rounded-l hover:bg-pink-200 transition-colors text-sm font-medium  cursor-pointer">
                                            Detail
                                        </button>
                                        ...
                                        <div
                                            ...
                                            x-init="$store.modal = { open: item => show(item) }"
                                            x-show="open"
                                            style="display: none;"
                                            class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                                            <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg relative">
                                                <button @click="close()" class="absolute top-2 right-2 text-pink-500 hover:text-pink-700 text-xl">&times;</button>
                                                <h2 class="text-xl font-bold mb-4 text-pink-700">Detail Barang Masuk</h2>
                                                <div class="mb-2"><span class="font-semibold">ID:</span> <span x-text="item.id"></span></div>
                                                <div class="mb-2"><span class="font-semibold">Tanggal:</span> <span x-text="item.created_at"></span></div>
                                                <div class="mb-2"><span class="font-semibold">Supplier:</span> <span x-text="item.supplier"></span></div>
                                                <div class="mb-2"><span class="font-semibold">Total Item:</span> <span x-text="item.total_item"></span></div>
                                                <div class="mb-2"><span class="font-semibold">Total Harga:</span> Rp <span x-text="item.total_price"></span></div>
                                                <div class="mb-4"><span class="font-semibold">Status Pembayaran:</span>
                                                    <span x-show="item.is_paid" class="text-green-600 font-semibold">Dibayar</span>
                                                    <span x-show="!item.is_paid" class="text-red-600 font-semibold">Belum Dibayar</span>
                                                </div>
                                                <template x-if="item.details && item.details.product && item.details.product.length">
                                                    <div>
                                                        <h3 class="font-semibold text-pink-600 mb-2">Produk:</h3>
                                                        <ul class="list-disc pl-5">
                                                            <template x-for="prod in item.details.product" :key="prod.id">
                                                                <li>
                                                                    <span x-text="prod.name"></span>
                                                                    (<span x-text="prod.qty"></span> x Rp <span x-text="prod.price"></span>)
                                                                </li>
                                                            </template>
                                                        </ul>
                                                    </div>
                                                ...
                                                <template x-if="!item.details || !item.details.product || !item.details.product.length">
                                                    <div class="text-gray-500">Tidak ada detail produk.</div>
                                                </template>
                                            </div>
                                        </div>
                                        <a :href="'/items-in/' + row.id + '/edit'" class="bg-pink-500 text-white px-3 py-1 hover:bg-pink-600 transition-colors text-sm font-medium">Edit</a>
                                        <form :action="'/items-in/' + row.id" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                    ...
                    <template x-if="!paged.length">
                        <tr>
                            <td colspan="7" class="border border-pink-300 px-4 py-6 text-center text-pink-500 font-semibold">Tidak ada data barang masuk.</td>
                        </tr>
                    </template>
                </tbody>
            </table>
