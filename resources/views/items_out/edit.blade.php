...
<main>
    <section class="flex flex-col items-start justify-start p-4 rounded-lg shadow-md bg-white border border-pink-200">
        ...
        <form action="{{ route('items-out.update',['id'=>$id]) }}" method="POST" class="w-full" x-data="{
        products: {{ Illuminate\Support\Js::from($products) }},
        items:  {{ Illuminate\Support\Js::from($itemOut->details) }},
        submit() {
                ...
                <div class="flex flex-col w-fit">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <label class="flex items-center space-x-2">
                            <input type="radio" name="status" value="{{ $enum->value }}" {{ old('status') == $enum->value || $itemOut->status->value == $enum->value ? 'checked' : '' }} class="form-radio text-pink-600">
                            <span>{{ $enum->name }}</span>
                        </label>
                        @endforeach
                    </div>
