...
<main x-data="{
    ...
    sortBy: 'id',
    sortAsc: true,
    items_in: {{ Illuminate\Support\Js::from($itemsOut->map(function($itemOut) {
            return [
                'id' => $itemOut->id,
                'created_at' => $itemOut->created_at->format('d/m/Y H:i'),
                'status' => $itemOut->status,
                'total_item' => $itemOut->total_item,
                'note' => $itemOut->note,
                'details' => [
                    'product' => $itemOut->details->map(function($detail) {
                        return [
                    ...
                    'total_price' => number_format($itemOut->details->sum(fn($d) => $d->quantity * $d->price), 0, ',', '.'),
                    ]
            ];
        })) }},
    page: 1,
    ...
    <section class="flex flex-col gap-6 items-start justify-start p-6 rounded-xl shadow-lg bg-white border border-pink-200 w-full max-w-6xl mx-auto mt-6">
        <div class="flex flex-col md:flex-row w-full justify-between items-center gap-4">
            <a href="{{ route('items-out.create') }}" class="bg-pink-500 text-white px-6 py-3 rounded-lg shadow hover:bg-pink-600 transition-colors font-semibold">
        ...
        <div class="overflow-x-auto w-full">
            <table class="w-full table-auto border-collapse rounded-lg overflow-hidden shadow border border-pink-300">
                ...
                <tbody>
                    <template x-if="paged.length">
                        <template x-for="row in paged" :key="row.id">
                            <tr class="hover:bg-pink-50 transition-colors">
                                ...
                                <td class="border border-pink-300 px-4 py-2" x-text="row.status"></td>
                                <td class="border border-pink-300 px-4 py-2 text-center" x-text="row.total_item"></td>
                                <td class="border border-pink-300 px-4 py-2 text-center" x-text="row.note"></td>
                                    ...
                                    <div class="inline-flex">
                                        ...
                                        <div
                                            ...
                                            <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg relative">
                                                ...
                                                <div class="mb-2"><span class="font-semibold">Status:</span> <span x-text="item.status"></span></div>
                                                <div class="mb-2"><span class="font-semibold">Total Item:</span> <span x-text="item.total_item"></span></div>
                                                <div class="mb-2"><span class="font-semibold">Catatan:</span> <span x-text="item.note"></span></div>
                                                        ...
                                                        <ul class="list-disc pl-5">
                                                            <template x-for="prod in item.details.product" :key="prod.id">
                                                                <li>
                                                                    ...
                                                                    (<span x-text="prod.qty"></span> : <span x-text="prod.note"></span>)
                                                                </li>
                                                            </template>
                                                        </ul>
                                                    </div>
                                        ...
                                        <a :href="'/items-out/' + row.id + '/edit'" class="bg-pink-500 text-white px-3 py-1 hover:bg-pink-600 transition-colors text-sm font-medium">Edit</a>
                                        <form :action="'/items-out/' + row.id" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                    ...
                    <template x-if="!paged.length">
                        <tr>
                            <td colspan="7" class="border border-pink-300 px-4 py-6 text-center text-pink-500 font-semibold">Tidak ada data barang keluar.</td>
                        </tr>
                    </template>
                </tbody>
            </table>
