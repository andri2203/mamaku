...
<main>
    <section class="flex flex-col items-start justify-start p-4 rounded-lg shadow-md bg-white border border-pink-200">
        ...
        <form action="{{ route('member.store') }}" @submit="loading = true" method="POST" class="w-full">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="phone" name="phone" value="{{ old('phone') }}" class="w-full px-3 py-2 border border-gray-300 rounded" required>
                @error('phone')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
