...
<main class="flex flex-col items-start justify-start w-full" x-data="{
    ...
    total_payment:0,
    items: {{  Illuminate\Support\Js::from($transaction->details->map(function ($item) {
    ...
    addItem(product) {
        ...
        if(existingIndex !== -1) {
            ...
            let total = this.items[existingIndex].subtotal;

            quantity += 1;
            total = quantity * this.items[existingIndex].product.price_sell;

            this.items[existingIndex].quantity = quantity;
            this.items[existingIndex].subtotal = total;
    ...
    increament(index){
        ...
        let total = this.items[index].subtotal;

        quantity += 1;
        total = quantity * this.items[index].product.price_sell;

        this.items[index].quantity = quantity;
        this.items[index].subtotal = total;
    },
    decreament(index){
        let quantity = this.items[index].quantity;
        let total = this.items[index].subtotal;
    ...
    get grandTotal() {
        ...
        let grandTotal = items.length == 0 ? 0 : items.length == 1? items[0].subtotal - items[0].discount : items.reduce((acc, item) => acc + (item.subtotal - item.discount), 0);
    ...
    <form action="{{ route('transaction.update', ['id' => $id]) }}" method="post" class="grid grid-cols-6 gap-4 w-full">
        @csrf
        @method('PUT')
        ...
        <section class="col-span-2 h-fit p-6 rounded-xl shadow-lg bg-white border border-pink-200">
            ...
            <div class="mb-4">
                <select id="member_id" name="member_id" class="w-full px-3 py-2 border border-gray-300 rounded">
                    ...
                    <option value="{{ $member->id }}" {{ $transaction->member != null && $transaction->member->id == $member->id ? 'selected' : '' }}>
                        {{ $member->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="text" id="total_item" x-model="totalItem" name="total_item" class="w-full px-3 py-2 border border-gray-300 rounded" readonly>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" id="discount" x-ref="discount" name="discount" value="{{ $transaction->discount }}" class="w-full px-3 py-2 border border-gray-300 rounded" x-on:change="getDiscount()">
                @error('discount')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" id="total_price" x-model="total_payment" name="total_price" class="w-full px-3 py-2 border border-gray-300 rounded" readonly>
            </div>
            ...
            <div class="flex flex-col w-fit mb-4">
                ...
                <div class="flex items-center space-x-4">
                    <label class="flex items-center space-x-2">
                        <input type="radio" name="is_paid" value="1" {{ $transaction->is_paid == '1' ? 'checked' : '' }} class="form-radio text-pink-600">
                        <span>Dibayar</span>
                    </label>
                    <label class="flex items-center space-x-2">
                        <input type="radio" name="is_paid" value="0" {{ $transaction->is_paid == '0' ? 'checked' : '' }} class="form-radio text-pink-600">
