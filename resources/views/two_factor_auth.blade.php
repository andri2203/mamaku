...
<body>
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-pink-100 via-purple-100 to-pink-200" x-data="{ loading: true }" x-init="setTimeout(() => loading = false, 1000)">
        ...
        <div class="backdrop-blur-md bg-white/70 border border-white/30 p-8 rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <!-- Title -->
            <h2 class="text-xl font-semibold text-center text-gray-800">Halaman Keamanan</h2>

            <!-- Form -->
            <form method="POST" action="{{ route('two_factor_auth.login') }}" @submit="loading = true">
                @csrf

                <p class="mb-4 text-gray-600 text-sm text-center">Anda memiliki kode keamanan. Mohon masukkan kode keamanan anda.</p>

                <!-- Kode Verifikasi -->
                <div class="mb-4">
                    <label for="verification_code" class="block text-gray-700 font-medium mb-2">Kode Keamanan</label>
            ...
            <!-- Logout -->
            <form method="POST" action="{{ route('logout') }}" class="pt-4 border-t border-pink-300">
                @csrf
